          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6">
            ...
            <div className="flex items-center gap-3">
              ...
              {ACTIVE_SOCIAL_LINKS.map(({ platform, url }) => {
                const footerIcons: Record<string, { svg: React.ReactNode; bg: string; hoverBg: string; iconColor: string }> = {
                  tiktok: {
                    bg: '#000000',
                    hoverBg: '#1a1a1a',
                    iconColor: '#ffffff',
                    svg: <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1v-3.5a6.37 6.37 0 00-.79-.05A6.34 6.34 0 003.15 15.2a6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.34-6.34V8.78a8.22 8.22 0 004.76 1.51V6.84a4.85 4.85 0 01-1-.15z"/></svg>,
                  },
                  instagram: {
                    bg: '#E4405F',
                    hoverBg: '#c73650',
                ...
                const iconData = footerIcons[platform];
                ...
                return (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="group relative flex items-center justify-center w-10 h-10 rounded-full transition-all duration-300 shadow-md hover:shadow-lg hover:scale-110"
                    style={{ backgroundColor: iconData.bg }}
                    onMouseEnter={(e) => {
                      (e.currentTarget as HTMLElement).style.backgroundColor = iconData.hoverBg;
                    }}
                    onMouseLeave={(e) => {
                      (e.currentTarget as HTMLElement).style.backgroundColor = iconData.bg;
                    }}
                    aria-label={platform.charAt(0).toUpperCase() + platform.slice(1)}
                  >
                    <span style={{ color: iconData.iconColor }}>{iconData.svg}</span>
                  </a>
                );
              })}
            </div>
